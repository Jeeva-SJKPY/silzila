        ...
        if (queries.size() == 1) {
            ...
            isOverrideMeasure.put(Meas.getMeasureOrder(),true); 
                ...
                for (Dimension dim : baseDimensions) {
                    ...
                    Dimension newDim = new Dimension(dim.getIsCalculatedField(), dim.getCalculatedField(), dim.getTableId(),dim.getFieldName() , dim.getDataType(), dim.getTimeGrain(),dim.isRollupDepth());
